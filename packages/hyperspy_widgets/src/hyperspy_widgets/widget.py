...
import numpy as np
from hyperspy_events import Event, Events
from matplotlib.backend_bases import MouseEvent, PickEvent

from ._axis_source import (
    AxisSourceBinding,
    axis_step,
    index_position_from_axes,
    position_from_index_values,
)
from ._defaults import preferences
from ._utils import on_figure_window_close
...
class WidgetBase(object):
    ...
    def __init__(self, axis_source=None, color="red", alpha=1.0, **kwargs):
        legacy_axis_source = kwargs.pop("axes_manager", None)
        if axis_source is None:
            axis_source = legacy_axis_source
        self.axis_source = axis_source
        self._axis_binding = AxisSourceBinding(axis_source)
        self._axes = list()
        self.ax = None
        self.picked = False
        self.selected = False
        ...
        self._axis_source_connected = False
    ...
    def connect(self, ax):
        ...
        if self._axis_source_connected:
            self.connect_axis_source()

    def connect_axis_source(self):
        """Connect widget position to an optional external axis source."""
        if self._axis_source_connected:
            self.disconnect_axis_source()
        self._axis_binding.connect(self._on_axis_source_change)
        self._on_axis_source_change(self.axis_source)
        self._axis_source_connected = True

    def disconnect_axis_source(self):
        """Disconnect a previous axis-source connection."""
        self._axis_binding.disconnect(self._on_axis_source_change)
        self._axis_source_connected = False

    def _on_axis_source_change(self, axis_source):
        """Callback for axis-source change notification."""
...
class DraggableWidgetBase(WidgetBase):
    """Adds the `position` and `index_position` properties, and adds a framework for
    ...
    def __init__(self, axis_source=None, **kwargs):
        super(DraggableWidgetBase, self).__init__(axis_source=axis_source, **kwargs)
        ...
        if self.axis_source is not None:
            self.axes = self._axis_binding.default_axes(1)
        else:
            self._pos = np.array([0.0])
    ...
    def _get_index_position(self):
        """Returns the position in axis-index units."""
        return index_position_from_axes(self.axes, self._pos)

    def _set_index_position(self, value):
        """Sets the position of the widget (by index units). The dimensions should
        ...
        if np.ndim(value) == 0 and len(self.axes) == 1:
            ...
            self.position = position_from_index_values(self.axes, value)

    index_position = property(
        lambda s: s._get_index_position(), lambda s, v: s._set_index_position(v)
    )
    ...
    def _pos_changed(self):
        ...
        """
        if self._axis_source_connected:
            self._axis_binding.push_position(
                self.axes, self._pos, callback=self._on_axis_source_change
            )
    ...
    def _on_axis_source_change(self, axis_source):
        if axis_source is self.axis_source:
            self.position = self._axis_binding.pull_position(self.axes)
...
class Widget1DBase(DraggableWidgetBase):
    ...
    position, adds the 'border_thickness' attribute and initializes the 'axes'
    attribute from the configured axis source. Other than that, it mainly supplies common
...
class ResizableDraggableWidgetBase(DraggableWidgetBase):
    ...
    functions include `get_centre` and `get_centre_in_index_units` which return the
    ...
    def __init__(self, axis_source=None, **kwargs):
        super(ResizableDraggableWidgetBase, self).__init__(
            axis_source=axis_source, **kwargs
        )
        if not self.axes:
    ...
    def _get_step(self, axis):
        """Use the local step size for uniform or non-uniform axes."""
        return axis_step(axis)
    ...
    def get_size_in_index_units(self):
        """Gets the size property converted to axis-index units (via 'axes'
        attribute).
        """
        s = list()
    ...
    def set_size_in_index_units(self, value):
        """Sets the size property converted to axis-index units (via 'axes'
        attribute).
        """
        s = list()
        for i in range(len(self.axes)):
    ...
    def get_centre(self):
        """Gets the center in axis values. The default implementation is simply the
    ...
    def get_centre_in_index_units(self):
        """Gets the center position in axis-index units. The default
        implementation is simply the index position + half the size, which should
        ...
        """
        return self.index_position + self.get_size_in_index_units() / 2.0
    ...
    def _apply_changes(self, old_size, old_position):
        ...
        if moved and self._axis_source_connected:
            self._axis_binding.push_position(
                self.axes, self._pos, callback=self._on_axis_source_change
            )
        if moved or resized:
...
class Widget2DBase(ResizableDraggableWidgetBase):
    ...
    def __init__(self, axis_source=None, **kwargs):
        super(Widget2DBase, self).__init__(axis_source=axis_source, **kwargs)
        self.border_thickness = 2

        # Set default axes
        if self.axis_source is not None:
            self.axes = self._axis_binding.default_axes(2)
        else:
            self._pos = np.array([0, 0])
