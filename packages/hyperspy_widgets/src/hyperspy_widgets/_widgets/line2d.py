...
from .._utils import picker_kwargs
from ..widget import ResizableDraggableWidgetBase
...
class Line2DWidget(ResizableDraggableWidgetBase):
    ...
    def __init__(self, axis_source=None, **kwargs):
        super(Line2DWidget, self).__init__(axis_source=axis_source, **kwargs)
        self.linewidth = 1
        ...
        value = self.axes[0].scale if self.axis_source else 1
        # [[x0, y0], [x1, y1]]
    ...
    def connect_axis_source(self):
        raise NotImplementedError("2D lines cannot be linked back to an axis source.")
    ...
    def _set_snap_size(self, value):
        if value and self.axes[0].scale != self.axes[1].scale:
            _logger.warning(
                "Snapping the width of the line is not supported "
    ...
    def _get_width_indicator_coords(self):
        ...
        """
        n = self.size[0] * self._get_line_normal() / 2.0
    ...
    def _rotate(self, event):
        ...
        w2 = np.array(
            (
                w1[:, 0] * np.cos(theta) - w1[:, 1] * np.sin(theta),
